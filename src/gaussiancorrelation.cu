                                                  ...
                                                  double sigma, bool auto_correlation, ThreadCtx &ctx)
{
...
#ifdef PROFILE_GAUSSIAN
    xf.sqr_norm(xf_sqr_norm,ctx.profData.getProfDevicePointer(ProfCUDA::KER_XF_SQR_NORM));
#else
    ...
    if (auto_correlation) {
        ...
        DEBUG_PRINTM(yf);
#ifdef PROFILE_GAUSSIAN
        yf.sqr_norm(yf_sqr_norm, ctx.profData.getProfDevicePointer(ProfCUDA::KER_YF_SQR_NORM));
#else
        yf.sqr_norm(yf_sqr_norm);
#endif
    }
#ifdef PROFILE_GAUSSIAN
    xyf = auto_correlation ? xf.sqr_mag() : xf.mulProf(yf.conj(ctx.profData.getProfDevicePointer(ProfCUDA::KER_YF_CONJ)), ctx.profData.getProfDevicePointer(ProfCUDA::KER_XF_MUL)); // xf.muln(yf.conj());
#else
...
#ifdef PROFILE_GAUSSIAN
    ComplexMat xyf_sum = xyf.sum_over_channels(ctx.profData.getProfDevicePointer(ProfCUDA::KER_XYF_SUM));
#else
